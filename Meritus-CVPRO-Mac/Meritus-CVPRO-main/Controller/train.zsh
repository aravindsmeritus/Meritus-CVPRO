...
userProfile="$HOME/Meritus-CVPRO-Mac/Meritus-CVPRO-main/Training_Process"
cd "$userProfile"

clear

while true; do
    echo "Is your Bot Disconnected from the Application and System? - '${BLUE}y${NC}' or '${BLUE}n${NC}': "
    read userInput
    if [[ "$userInput" == "y" ]]; then
        echo "${GREEN}Initializing the Training Process!${NC}"
        break
    elif [[ "$userInput" == "n" ]]; then
        echo "${BLUE}Please disconnect the Bot from your Application and the System!${NC}"
        return 1
    else
