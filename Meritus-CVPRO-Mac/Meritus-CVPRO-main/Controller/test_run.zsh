...
userProfile="$HOME/Meritus-CVPRO-Mac/Meritus-CVPRO-main/Controller"
cd "$userProfile"
...
run_bot() {
    local script="$1"
    while true; do
        echo "Is your Bot connected with the Application? - '${BLUE}y${NC}' or '${BLUE}n${NC}': "
        read userInput
        if [[ "$userInput" == "y" ]]; then
    ...
    python "$script"
    echo
    echo "Type '${BLUE}run_cvpro${NC}' or '${BLUE}train_cvpro${NC}' according to your requirement!"
}
